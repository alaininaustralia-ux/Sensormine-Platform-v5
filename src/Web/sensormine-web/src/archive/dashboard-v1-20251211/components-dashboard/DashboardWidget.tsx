...
export function DashboardWidgetComponent({
  ...
  const renderWidgetContent = () => {
    ...
    // Check if chart has sample/default data in config
    const hasChartData = widget.type === 'chart' && widget.config?.series && widget.config.series.length > 0;

    // If configured or has sample data, use appropriate renderer
    if (hasDataConfig) {
      return <WidgetDataRenderer widget={widget} deviceId={deviceId} dashboardId={dashboardId} />;
    }

    // For charts with sample data, render directly
    if (hasChartData) {
      const TimeSeriesChart = require('./widgets/charts/time-series-chart').TimeSeriesChart;
      return <TimeSeriesChart config={widget.config} />;
    }
